...
/**
 * Damage reward rendered as a badge.
 *
 ...
 * dealt. The host element is the badge itself.
 */
@Component({
  selector: 'app-damage-badge',
  imports: [TranslatePipe],
  template: '{{ damage() }} {{ "shared.damageBadge.unit" | translate }}',
  host: {
...
export class DamageBadge {
  ...
  protected readonly toneClass = computed(() =>
    this.damage() === 0 ? 'bg-surface-700 text-text-muted' : 'bg-accent-red/15 text-accent-red',
  );
}
