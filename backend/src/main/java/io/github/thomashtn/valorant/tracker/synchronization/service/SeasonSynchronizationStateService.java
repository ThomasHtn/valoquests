...
/**
 * Owns the per-player, per-season completion flag driving match-history pagination.
 *
 ...
 * imported, and {@link #markSeasonComplete} commits only after the page that proved the boundary was
 * itself committed. A crash at any point therefore leaves {@code complete = false} with a partial
 * history, which the next run repairs; {@code complete = true} with missing pages is unreachable.
 *
...
public class SeasonSynchronizationStateService {
     ...
     * flag and a re-walk of an interrupted season keeps its {@code false}.
     *
     * @param player tracked player
     * @param season season about to be walked
     * @return the local season identifier
     */
    @Transactional
    public Long startSeason(Player player, Season season) {
        return stateRepository
            .findByPlayerIdAndSeasonId(player.getId(), season.getId())
            .map(existing -> existing.getSeason().getId())
            .orElseGet(() -> create(player, season));
    }
